...
async function resetSequence(modelName: string) {
  ...
  const maxIdResult = await (
    (prisma as any)[toCamelCase(toPascalCase(modelName))]
  ).findMany({
    select: { id: true },
    orderBy: { id: "desc" },
    take: 1,
...
async function main() {
  // Use project root to find seedData. Adjust if your seedData lives elsewhere.
  ...
  for (const fileName of orderedFileNames) {
    ...
    if (modelName === "Location") {
      ...
      if (!model) {
        console.error(
          `Prisma model ${modelNameCamel} not found; skipping ${fileName}`
        );
        continue;
      }
      try {
